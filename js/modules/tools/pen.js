  ...
  item.addEventListener('click', () => {
    ...
    data.canv.addEventListener('mouseup', (e) => {
      if (data.tools.isPen === true) {
        mouseDown = false;
        x1 = Math.floor(e.offsetX / data.canvIndexX);
        y1 = Math.floor(e.offsetY / data.canvIndexY);
      }
    });
    ...
    data.canv.addEventListener('mousedown', (e) => {
      ...
      if (data.tools.isPen === true) {
        mouseDown = true;
        x0 = Math.floor(e.offsetX / data.canvIndexX);
        y0 = Math.floor(e.offsetY / data.canvIndexY);
        drawRect(x0, y0);
      }
    });
    ...
    data.canv.addEventListener('mousemove', (e) => {
      ...
      if (mouseDown && data.tools.isPen === true) {
        if (Math.abs(x / data.canvIndexX - x0) > 2 || Math.abs(y / data.canvIndexY - y0) > 2) {
          x1 = Math.floor(e.offsetX / data.canvIndexX);
          y1 = Math.floor(e.offsetY / data.canvIndexY);
